...
class Particle {
  ...
  void fall() {
    try {
      ...
      if (y > scaledResolution.getHeight()) y = 1;

      if (x > scaledResolution.getWidth()) x = 1;

      if (x < 1) x = scaledResolution.getScaledWidth();

      if (y < 1) y = scaledResolution.getScaledHeight();
    } catch (Exception E) {
      E.printStackTrace();
    }
  }
