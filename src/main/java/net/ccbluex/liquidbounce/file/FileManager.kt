...
class FileManager : MinecraftInstance() {
    val dir = File(mc.runDirectory, "FDPCLIENT-1.8")
    ...
    fun loadBackground() {
        if (backgroundFile.exists()) {
            try {
                ...
                LiquidBounce.background = Identifier(LiquidBounce.CLIENT_NAME.lowercase() + "/background.png")
                mc.textureManager.registerTexture(LiquidBounce.background, DynamicTexture(bufferedImage))
